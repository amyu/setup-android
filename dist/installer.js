...
import { ANDROID_SDK_ROOT, COMMANDLINE_TOOLS_LINUX_URL, COMMANDLINE_TOOLS_MAC_URL, COMMANDLINE_TOOLS_WINDOWS_URL } from './constants.js';
export async function installAndroidSdk(versions) {
    ...
    const extractedCmdlineToolPath = await toolCache.extractZip(downloadedCmdlineToolsPath, path.join(ANDROID_SDK_ROOT, 'cmdline-tools'));
    ...
    if (versions.buildToolsVersion.length > 0) {
        ...
        core.info(`start install platform-tools and sdk:${versions.sdkVersion} (build-tools skipped)`);
    }
    ...
    if (versions.buildToolsVersion.length > 0) {
        ...
        core.info(`success install platform-tools and sdk:${versions.sdkVersion} (build-tools skipped)`);
    }
